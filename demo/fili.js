/**
 * @name    fili
 * @version 1.2.0 | November 12th 2015
...
},{"./src/calcCascades":2,"./src/fft":3,"./src/firCoeffs":4,"./src/firFilter":5,"./src/iirCoeffs":6,"./src/iirFilter":7,"./src/testFilter":8,"./src/wt":10}],2:[function(require,module,exports){
'use strict';
...
var Wt = function Wt(params) {
  ...
  var getCoefficients = function getCoefficients(radix) {
  ...
  var _resetBuffer = function _resetBuffer() {
    for (var cnt = 0; cnt < _waveletBuffer.length; cnt++) {
      _waveletBuffer[cnt].lowpassPointer = 0;
    }
    waveData.lowpassPointer = 0;
  };

  var calculateCWT = function calculateCWT() {
    for (var cnt = 0; cnt < waveletDepth; cnt++) {
      var buffer = _waveletBuffer[cnt];
      var data = waveData;
      if (cnt > 0) {
        data = _waveletBuffer[cnt - 1];
      }
  ...
  var calculateDWT = function calculateDWT() {
    for (var cnt = 0; cnt < waveletDepth; cnt++) {
      var buffer = _waveletBuffer[cnt];
      var data = waveData;
      if (cnt > 0) {
  ...
  var setDepth = function setDepth(depth) {
    if (depth !== 2 && depth !== 4 && depth !== 6 && depth !== 8 && depth !== 10) {
      return false;
    }
    waveletCoeffs = getCoefficients('D' + depth);
    if (depth > _waveletBuffer.length) {
      for (var cnt = _waveletBuffer.length; cnt < depth; cnt++) {
        _waveletBuffer.push({
    ...
    } else if (depth <= _waveletBuffer.length) {
      _waveletBuffer.length = depth;
    }
    waveletDepth = depth;
    _resetBuffer();
    return true;
  ...
  var calculateNecessarySamples = function calculateNecessarySamples() {
